// Simple DNS extractor - just DNS records, no link tracking
(async function extractDNSRecords() {
  const sleep = ms => new Promise(r => setTimeout(r, ms));
  
  console.log('=== SIMPLE CIO DNS EXTRACTOR ===');
  ...
  try {
    ...
    const encodedData = btoa(JSON.stringify(finalResults));
    const url = `https://cio-dns.netlify.app/?data=${encodeURIComponent(encodedData)}`;
    ...
    alert(`Error extracting DNS records: ${error.message}`);
  }
})();